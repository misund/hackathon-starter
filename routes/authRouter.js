...
authRouter.get('/instagram/callback', passport.authenticate('instagram', { failureRedirect: '/login' }), (req, res) => {
...
authRouter.get('/foursquare/callback', passport.authorize('foursquare', { failureRedirect: '/api' }), (req, res) => {
  res.redirect('/api/foursquare');
});
authRouter.get('/tumblr', passport.authorize('tumblr'));
authRouter.get('/tumblr/callback', passport.authorize('tumblr', { failureRedirect: '/api' }), (req, res) => {
...
authRouter.get('/venmo/callback', passport.authorize('venmo', { failureRedirect: '/api' }), (req, res) => {
  res.redirect('/api/venmo');
});
authRouter.get('/steam', passport.authorize('openid', { state: 'SOME STATE' }));
authRouter.get('/steam/callback', passport.authorize('openid', { failureRedirect: '/login' }), (req, res) => {
  res.redirect(req.session.returnTo || '/');
});
authRouter.get('/pinterest', passport.authorize('pinterest', { scope: 'read_public write_public' }));
authRouter.get('/pinterest/callback', passport.authorize('pinterest', { failureRedirect: '/login' }), (req, res) => {
